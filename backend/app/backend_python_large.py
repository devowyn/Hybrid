...
def width_penalty(width, length):
    # Large vehicle penalty (width 1.8–2.1m)
    # Narrow roads get HIGH penalties, wide roads get NO penalty
    if pd.isna(width) or pd.isna(length):
        return length * 0.5   # unknown width — small precautionary penalty
    elif width < 2.1:
        return length * 15    # too narrow for SUV — very high penalty
    elif width < 3.0:
        return length * 8     # tight fit — high penalty
    elif width < 4.0:
        return length * 2     # passable but tight — moderate penalty
    else:
        return 0              # wide enough — no penalty
...
if __name__ == "__main__":
    ...
    uvicorn.run(
        "backend_python_large:app",
        host="0.0.0.0",
        port=8001,   # different port from small vehicle backend
        reload=True
    )
